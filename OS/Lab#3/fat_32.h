#ifndef LAB_3_FAT32_H
#define LAB_3_FAT32_H
...
private:
    ...
    uint32_t BPB_TotSec32; //> 65536

    uint32_t BPB_FATSz32; //количество секторов одной FAT.
    uint16_t BPB_ExtFlags = 0xFFFF;
...
public:
    fat_32(
            ...
            uint32_t BPB_TotSec=400000,
            uint16_t BPB_FATSz=18,
        ...
        {
            throw ConstructorException("BPB_TotSec32 must be more than 65536");
        }

        this->BPB_FATSz32 = BPB_FATSz;

        unsigned DataSec = BPB_TotSec - (BPB_RsvdSecCnt + (BPB_NumFATs * BPB_FATSz));
        unsigned CountOfClusters = DataSec / BPB_SecPerClus;
        if (CountOfClusters < 65525) {
            throw ConstructorException("This is not FAT32\nTry more or less clusters");
        }
    ...
    void printBootSector() {
        ...
        this->printInt32(this->BPB_FATSz32);
        this->printInt16(this->BPB_ExtFlags);
        this->printInt16(this->BPB_FSVer);
        this->printInt32(this->BPB_RootClus);
        this->printInt16(this->BPB_FSInfo);
        this->printInt16(this->BPB_BkBootSec);
...
public:
    ...
    void bootFromFile(std::string imageName) {
        ...
        this->BPB_FATSz32 = getInt32();
        this->BPB_ExtFlags = getInt16();
        this->BPB_FSVer = getInt16();
        this->BPB_RootClus = getInt32();
        this->BPB_FSInfo = getInt16();
        this->BPB_BkBootSec = getInt16();
...
private:
    ...
    void printFatTable() {
        uint32_t freeCluster = 0x00000000, lastCluster = 0xFFFFFFFF, reservedValue = 0xFFFFFFF8;
        this->printInt32(reservedValue);
        this->printInt32(lastCluster);
    }

    void editFatTableCluster(int cluster, uint32_t value) {
        for (int i = 0; i < this->BPB_NumFATs; i++) {
            fseek(vhd, this->BPB_BytsPerSec * (i * this->BPB_FATSz32 + this->BPB_RsvdSecCnt)
                       + cluster * 4, SEEK_SET);
            this->printInt32(value);
        }
    }

    int getNextCluster(int cluster) {
        fseek(vhd, this->BPB_BytsPerSec * BPB_RsvdSecCnt + cluster * 4, SEEK_SET);
        return getInt32();
    }

    int getFreeCluster() {
        for (int cluster = 3; cluster < this->BPB_FATSz32 * this->BPB_BytsPerSec; cluster++) {
            fseek(vhd, this->BPB_BytsPerSec * BPB_RsvdSecCnt + cluster * 4, SEEK_SET);
            int clusterInfo = getInt32();
            if (clusterInfo == 0x00000000) {
                return cluster;
            }
        }
    ...
    int getFreeClusterSpaceOffset(int cluster) {
        ...
        for (int i = this->BPB_SecPerClus * this->BPB_BytsPerSec - 32; i >= 0; i -= 32) {
            fseek(vhd, this->BPB_BytsPerSec * (BPB_NumFATs * this->BPB_FATSz32 + this->BPB_RsvdSecCnt)
...
//        4 bytes size
        ...
        while (nameOffset < fullName.length()) {
            try {
                ...
                if (cluster == 0xFFFFFFFF) {
                    ...
                    editFatTableCluster(newCluster, 0xFFFFFFFF);
                }
                continue;
            }
        }
    ...
    int printFileData(const std::string& path) {
        ...
        for (int i = 0; i < size || i == 0; i += clusterSize) {
            ...
            editFatTableCluster(freeCluster, 0xFFFFFFFF);
    ...
    void printDirectoryContent(const std::string& path, int firstCluster, int parentCluster = -1) {
        if (parentCluster == -1) {
            editFatTableCluster(firstCluster, 0xFFFFFFFF);
        }
        int entCount = 0;
        int curCluster = firstCluster;
        if (parentCluster != -1) {
        ...
        for (auto& entry : fs::directory_iterator(path)) {
            ...
            if (fs::is_directory(entry.path())) {
                ...
                editFatTableCluster(dataFirstCluster, 0xFFFFFFFF);
...
public:
    ...
    void printImage(std::string filename="b0lgenO5_FAT16.vhd", std::string pathToDir="") {
        ...
        for (int i = 0; i < BPB_NumFATs; i++) {
            fseek(vhd, this->BPB_BytsPerSec * (this->BPB_FATSz32 * i + this->BPB_RsvdSecCnt), SEEK_SET);
            printFatTable();
        }

        if (!pathToDir.empty()) {
            printDirectoryContent(pathToDir, 2);
        }
    }
...
private:
    ...
    int findClusterWithDirectory(const std::string& pathInside, int cluster = 2) {
        std::string buff;
