#ifndef LAB_3_FAT16_H
#define LAB_3_FAT16_H
...
public:
                    ...
                    )
    {
        ...
        if (BPB_SecPerClus != 1 && BPB_SecPerClus != 2 && BPB_SecPerClus != 4 && BPB_SecPerClus != 8 && BPB_SecPerClus != 16 && BPB_SecPerClus != 32 && BPB_SecPerClus != 64 && BPB_SecPerClus != 128) {
        ...
        if (CountOfClusters < 4085 || CountOfClusters >= 65525) {
            throw ConstructorException("This is not FAT16\nTry more or less clusters");
        }
...
public:
    ...
    void bootFromFile(std::string imageName) {
        ...
        this->BS_jmpBoot = getVector(3);//fix
        this->BS_OEMName = getVector(8);//fix
...
private:
    ...
    void printFatTable() {
        uint16_t freeCluster = 0x0000, lastCluster = 0xFFFF, reservedValue = 0xFFF8;
        this->printInt16(reservedValue);
        this->printInt16(lastCluster);
    }
...
//        4 bytes size
        ...
        while (nameOffset < fullName.length()) {
            try {
                ...
                if (cluster == 0xFFFF) {
                    ...
                    editFatTableCluster(newCluster, 0xFFFF);
                }
                continue;
            }
        }
    ...
    int printFileData(const std::string& path) {
        ...
        for (int i = 0; i < size || i == 0; i += clusterSize) {
            ...
            editFatTableCluster(freeCluster, 0xFFFF);
    ...
    void printDirectoryContent(const std::string& path, int firstCluster, int parentCluster = -1) {
        ...
        for (auto& entry : fs::directory_iterator(path)) {
            ...
            if (fs::is_directory(entry.path())) {
                ...
                editFatTableCluster(dataFirstCluster, 0xFFFF);
